    ...
    <div class="bg-gray-100 h-full">
        <div class="flex justify-between items-center bg-amber-300 p-4 shadow-md sticky top-0">
            <div class="text-gray-500 flex items-center gap-x-2">
                ...
                <h1 class="text-gray-500 text-xl">Tambah Bengkel</h1>
            </div>
        </div>
        <form action="/bengkel/form_tambah/tambah_bengkel" method="POST" enctype="multipart/form-data">
            <div class="w-full flex justify-center">
                <div class="max-w-lg w-full leading-normal">
                    <div class="m-8 pb-20 space-y-4">
                        ...
                        <input type="hidden" name="nama_pemilik" id="" value="<?= session()->get('nama_pemilik'); ?>">
                        <input type="hidden" name="no_telp" id="" value="<?= session()->get('no_telp'); ?>">
                        <input type="hidden" name="username" id="" value="<?= session()->get('username'); ?>">
                        <input type="hidden" name="password" id="" value="<?= session()->get('password'); ?>">
                        <input type="hidden" name="whatsapp" id="" value="<?= session()->get('whatsapp'); ?>">
                        <div class="w-full">
                            <label for="" class="block">Nama Bengkel</label>
                            <input class="w-full bg-gray-200 py-2 px-4 rounded-lg outline-none border border-gray-400" type="text" name="nama_bengkel" placeholder="masukan nama bengkel">
                        </div>
                        <div>
                            <label class="block" for="">Gambar</label>
                            <input class="w-full bg-gray-200 rounded-lg border border-gray-400" name="gambar" type="file">
                        </div>
                        <div class="flex items-center justify-center">
                            <div class="w-full">
                                <label class="block" for="">Jam Buka</label>
                                <input class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400" type="time" name="jam_buka" id="">
                            </div>
                            <div class="w-full">
                                <label class="block" for="">Jam Tutup</label>
                                <input class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400" type="time" name="jam_tutup">
                            </div>
                        </div>
                        <div>
                            <label class="block" for="">Alamat</label>
                            <textarea name="alamat" id="" class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400 outline-none w-full"></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400 outline-none" type="text" name="lat" id="">
                        </div>
                        <div>
                            <label class="block" for="">Longitude</label>
                            <input class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400 outline-none" name="long" type="text">
                        </div>
                        <div>
                            <label class="block" for="">Link Lokasi Bengkel</label>
                            <textarea class="bg-gray-200 rounded-lg py-2 px-4 border border-gray-400 outline-none w-full" name="link" type="text"></textarea>
                        </div>
                    </div>
                </div>
            </div>
            <button type="submit" class="fixed bottom-20 right-8 bg-amber-300 rounded-full p-4 shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
